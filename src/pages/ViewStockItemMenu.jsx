...
export default function ViewStockItemMenu() {
  ...
  const [stockItems, setStockItems] = useState([]);
      ...
      React.useEffect(() => {
          ...
          privateAxios.get(`/api/v1/stock-item-menu/all?pageNumber=${page}&pageSize=${rowsPerPage}`)
              .then(response => {
        ...
        <Table stickyHeader aria-label="sticky table">
          ...
          <TableBody>
            {stockItems?.content
              ...
              .map((row) => {
                return (
                  <TableRow hover role="checkbox" tabIndex={-1} key={row.accountCode} style={{ cursor: "pointer" }}  onClick={() => navigateToEdit(row.accountCode)}>
                    {columns.map((column) => {
